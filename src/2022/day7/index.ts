...
type TreeEntries = (TreeCommand | TreeFile | TreeDir)[];

type TreeDisk = {
    [key: string]: TreeDisk | number;
};
...
const recreateDisk = (entries: TreeEntries) => {
    const disk: TreeDisk = {};
    const path: string[] = [];

    const createFileOrDir = (name: string, val: number | TreeDisk = {}) => {
        const p = [...path];
        let cwd = disk;

        path.forEach((step) => {
            if (cwd[step] === undefined) {
                cwd[step] = {} as TreeDisk;
            }
            cwd = cwd[step] as TreeDisk;
        });
...
const parseInput = (rawInput: string) => {
    const entries: TreeEntries = rawInput.split('\n').map((line) => {
        if (line.startsWith('$')) {
            const [command, arg] = line.split(' ').slice(1);
            return { type: 'command', command, arg } as TreeCommand;
        }
